...
static sgx_status_t SGX_CDECL sgx_ecall_IIV_sample(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_IIV_sample_t));
	ms_ecall_IIV_sample_t* ms = SGX_CAST(ms_ecall_IIV_sample_t*, pms);
	...
	ms->ms_retval = ecall_IIV_sample();


	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_ImagVerify(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_ImagVerify_t));
	ms_ecall_ImagVerify_t* ms = SGX_CAST(ms_ecall_ImagVerify_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_image_name = ms->ms_image_name;
	size_t _len_image_name = _tmp_image_name ? strlen(_tmp_image_name) + 1 : 0;
	char* _in_image_name = NULL;
	char* _tmp_image_path = ms->ms_image_path;
	size_t _len_image_path = _tmp_image_path ? strlen(_tmp_image_path) + 1 : 0;
	char* _in_image_path = NULL;

	CHECK_UNIQUE_POINTER(_tmp_image_name, _len_image_name);
	CHECK_UNIQUE_POINTER(_tmp_image_path, _len_image_path);
	...
	if (_tmp_image_name != NULL && _len_image_name != 0) {
		_in_image_name = (char*)malloc(_len_image_name);
		if (_in_image_name == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_image_name, _tmp_image_name, _len_image_name);
		_in_image_name[_len_image_name - 1] = '\0';
	}
	if (_tmp_image_path != NULL && _len_image_path != 0) {
		_in_image_path = (char*)malloc(_len_image_path);
		if (_in_image_path == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_image_path, _tmp_image_path, _len_image_path);
		_in_image_path[_len_image_path - 1] = '\0';
	}

	ms->ms_retval = ecall_ImagVerify(_in_image_name, _in_image_path);
err:
	if (_in_image_name) free(_in_image_name);
	if (_in_image_path) free(_in_image_path);
...
static sgx_status_t SGX_CDECL sgx_ecall_file_open(void* pms)
{
	...
	size_t _len_mode = _tmp_mode ? strlen(_tmp_mode) + 1 : 0;
	char* _in_mode = NULL;
	...
	if (_tmp_mode != NULL && _len_mode != 0) {
		...
		memcpy((void*)_in_mode, _tmp_mode, _len_mode);
...
err:
	...
	if (_in_mode) free((void*)_in_mode);
...
SGX_EXTERNC const struct {
	...
	5,
	{
		{(void*)(uintptr_t)sgx_ecall_IIV_sample, 0},
		{(void*)(uintptr_t)sgx_ecall_ImagVerify, 0},
...
sgx_status_t SGX_CDECL ocall_IIV_sample(const char* str)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_str = str ? strlen(str) + 1 : 0;

	ms_ocall_IIV_sample_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_IIV_sample_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_IIV_sample_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_IIV_sample_t));
...
sgx_status_t SGX_CDECL ocall_sha256_file(char* path, char* outputBuffer, size_t len)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_path = path ? strlen(path) + 1 : 0;
	size_t _len_outputBuffer = len;

	ms_ocall_sha256_file_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_sha256_file_t);
	void *__tmp = NULL;

	void *__tmp_outputBuffer = NULL;
	ocalloc_size += (path != NULL && sgx_is_within_enclave(path, _len_path)) ? _len_path : 0;
	ocalloc_size += (outputBuffer != NULL && sgx_is_within_enclave(outputBuffer, _len_outputBuffer)) ? _len_outputBuffer : 0;
	...
	ms = (ms_ocall_sha256_file_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_sha256_file_t));
	...
	if (outputBuffer != NULL && sgx_is_within_enclave(outputBuffer, _len_outputBuffer)) {
		ms->ms_outputBuffer = (char*)__tmp;
		__tmp_outputBuffer = __tmp;
		memset(__tmp_outputBuffer, 0, _len_outputBuffer);
		__tmp = (void *)((size_t)__tmp + _len_outputBuffer);
	} else if (outputBuffer == NULL) {
		ms->ms_outputBuffer = NULL;
	} else {
		sgx_ocfree();
		return SGX_ERROR_INVALID_PARAMETER;
	}
	...
	if (outputBuffer) memcpy((void*)outputBuffer, __tmp_outputBuffer, _len_outputBuffer);
